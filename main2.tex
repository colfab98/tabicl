...
The resulting informed target is then combined with the generic synthetic
target using the trial's target-mixture weight.
...
The new Optuna runner reuses the previous
\texttt{pitting\_magpie\_full\_v1} workflow rather than replacing the
underlying training implementation. The following settings were retained:
...
\begin{itemize}
    ...
    \item optional Magpie descriptor augmentation;
    \item the informed-prior ratio, MLP probability, feature-block strength,
    target-mixture weight, and material-generator parameters;
    \item eight-estimator trial evaluation; and
    \item mean development-fold Spearman correlation as the Optuna objective.
\end{itemize}

The old material, environment, and material--chloride coefficients were
removed from the Optuna search space. These coefficients are now determined
during Stage~2 calibration instead of being independently selected by every
Optuna trial.
...
\section{Final v6 Informed-Feature SCM Prior}

This section supersedes the preceding interim descriptions of the v5
EPIT-rule-only target. The final implementation retains the empirical physical
feature generator, but it obtains the informed target by passing those
features through the selected MLP or tree SCM. The implemented mode is

\[
\texttt{pitting\_composition\_mode}
=
\texttt{empirical\_features\_scm\_target}.
\]

\subsection{Likely Cause of the Trial 25 Underperformance}

Trial~25 was the best configuration in the
\texttt{fe\_ni\_softmax} Optuna study, but its strong performance during the
\(1{,}000\)-step proxy run did not transfer reliably to final training.
Changes in the data split and stochastic training may explain part of this
difference, but the most likely cause was a structural mismatch between the
synthetic prior and the real EPIT data.

The previous generator did not produce realistic alloy compositions. It
transformed dense synthetic values using a softmax and assigned the largest
component to Fe or Ni. The resulting rows distributed substantial mass across
many of the 17 element columns, whereas real Fe and Ni--Cr--Mo alloys are
typically dominated by a small number of principal elements. Real Fe alloys,
for example, are generally Fe-dominant, while the synthetic ``Fe alloys''
contained much less Fe and unrealistically large quantities of many minor
elements.

The problem was therefore not simply that the softmax distribution was too
flat. Increasing its sharpness would still place mass on arbitrary combinations
of elements. It would not reproduce the real concentration ranges, sparsity,
co-occurrence patterns, or relationships between Fe, Ni, Cr, Mo, and the minor
alloying elements.

The environmental variables had a related problem. Temperature, chloride
concentration, pH, and test method were generated or transformed separately.
Their individual ranges could appear plausible, but their joint combinations
did not necessarily correspond to experimental conditions represented in the
real dataset.

Trial~25 also selected a feature-block coupling strength of approximately
...
This forced columns within the material and environment blocks to share a
strong artificial latent component. Such coupling can be useful for generic
synthetic tables, but it is inappropriate for alloy compositions whose
dependence structure should arise from chemical composition and mass closure.
It could therefore create synthetic correlations that were absent from the
real EPIT data.

Magpie descriptors were also enabled in Trial~25. Because these descriptors
were calculated from unrealistic synthetic compositions, they could amplify
the distribution mismatch rather than correct it. Optuna may have benefited
from these descriptors during the short proxy run without that advantage
remaining stable during longer training.

A further limitation was the mismatch between Optuna and final-training
horizons. Trial~25 was selected after \(1{,}000\) training steps but was then
used in a substantially longer final-training run. A configuration that
performs best early is not necessarily the configuration that remains best
after \(10{,}000\) steps. This is particularly important when the informed
prior contains unrealistic regularities that the model can learn increasingly
strongly over time.

Finally, Trial~25 used eight parallel prior-generation jobs. Although this
increased data-generation throughput, it made exact reproduction of the
synthetic training stream less reliable.

These observations suggest that the underperformance was not caused primarily
by the EPIT target formulas themselves. The more likely problem was that the
formulas were applied to synthetic feature rows that did not resemble the
physical domain in which the formulas were intended to operate.

\subsection{Revised Order of Synthetic-Data Generation}

The v6 implementation separates two responsibilities. The empirical generator
determines what the informed features look like, while the SCM target head
determines how the target depends on those features. Each informed task first
receives a complete physical EPIT table. A standardized copy is then passed to
the selected MLP or tree target head:
...
Z_{\mathrm{target}}
\longrightarrow
f_{\theta}^{\mathrm{MLP/tree}}
\longrightarrow
y_{\mathrm{synthetic}}.
\]

The feature table returned for training remains
\(X_{\mathrm{physical}}\). The SCM receives the informed features as
externally supplied causes; it does not generate a second feature table and
does not modify the physical one. This preserves a direct relationship between
the generated informed features and their synthetic target.

\subsection{Empirical Alloy-Composition Generator}

The initial implementation is deliberately restricted to two alloy families:

\begin{itemize}
    \item Fe alloys; and
    \item Ni--Cr--Mo alloys.
\end{itemize}

Additional alloy families can be introduced later, but retaining these two
families makes the first comparison feasible and keeps the synthetic prior
aligned with the current EPIT evaluation scope.

Compositions are sampled from real templates in the existing
\texttt{epit\_dataset\_v1} profile. The restricted template bank contains

\[
298
\quad\text{Fe-alloy templates}
\]
...
Zero and unreported elements remain zero. The complete 24-element composition
is subsequently closed to \(100\) weight percent:
...
Consequently, the sum of the visible 17 elements can be below \(100\%\) when
part of the source composition belongs to elements omitted from the evaluated
schema.

This approach retains realistic dominant elements, approximate concentration
ranges, sparsity, and element co-occurrence patterns. It does not merely make
the previous softmax distribution sharper; it replaces that mechanism with
controlled perturbations around observed alloy compositions.

\subsection{Empirical Environmental-Condition Generator}
...
\[
\texttt{epit\_fe\_nicrmo\_features\_v1}
\]

is sampled with replacement. Temperature, chloride concentration, pH, and test
method are retained as a complete tuple from the same source row. This
preserves experimentally observed combinations of environmental conditions.

Composition and environment are sampled independently. The generator therefore
retains realistic composition structure and realistic environmental structure
without copying the dataset's existing association between a particular alloy
family and a particular experimental campaign.

Missing numerical values use the same means as the direct EPIT evaluator. Test
methods use the same lexicographically ordered 52-category encoding. The final
synthetic input always follows the fixed EPIT schema:

\[
17\text{ composition features}
+
3\text{ environmental features}
+
1\text{ test-method feature}.
\]

Constant-column removal is disabled for these informed tasks. Otherwise, a
rare or temporarily constant element could be deleted and cause temperature,
chloride concentration, pH, or test method to move into an incorrect column
position.

\subsection{Feed-Forward SCM Target from the Informed Features}

Before the target head is evaluated, a separate copy of each physical column is
standardized within the current synthetic dataset using the population mean and
standard deviation:
...
This prevents a high-magnitude column, such as temperature or an alloy
percentage, from dominating the randomly initialized target mapping solely
because of its units. The standardization is used only inside the target
calculation. It neither overwrites nor rescales the physical features returned
to TabICL.

Setting a constant column to zero is task-specific rather than a permanent
statement that the feature is irrelevant. A column with one value contains no
within-task information from which that task's target can vary. In another
synthetic task where the same column varies, it is standardized normally and
can influence the target. Across many generated tasks, TabICL can therefore
still learn to use that feature whenever variation is present.

When the MLP branch is selected, the standardized informed features are passed
through a direct non-causal feed-forward network:

\[
y_{\mathrm{synthetic}}
=
f_{\theta}^{\mathrm{MLP}}
\!\left(Z_{\mathrm{target}}\right).
\]
...
target value per row. Because the mode is direct and non-causal, the 21
supplied physical columns remain the causes and only the final network output
is used as \(y\).

When the tree branch is selected, the same standardized features are passed
through the existing direct tree-SCM transformation:
...
Thus, \texttt{mlp\_prob} selects between the MLP feed-forward target head and
the tree target head. It does not select whether the task is informed.
Informed versus generic task generation is controlled separately by
\texttt{informed\_prior\_ratio}.

The final informed pair is therefore
...
f_{\theta}^{\mathrm{MLP/tree}}
\!\left(
\operatorname{zscore}_{\mathrm{dataset}}
(X_{\mathrm{physical}})
\right)
\right).
\]

The seven analytical EPIT target rules and their former mixture with a generic
target are not used in this v6 mode. Consequently,
\texttt{informed\_target\_mix\_weight} is not an Optuna dimension, and the
v6 training command does not receive target-rule scores or coefficients.

\subsection{Generic and Informed Tasks in the Same Training Run}

The generic part of the prior remains unchanged. For each generated task,
\texttt{informed\_prior\_ratio} determines whether the standard generic SCM
path or the new informed path is used:

\[
\mathcal{D}_{\mathrm{train}}
\sim
\begin{cases}
\mathcal{D}_{\mathrm{informed}},
& \text{with probability }\rho,\\
\mathcal{D}_{\mathrm{generic}},
...
For an informed task, the MLP/tree choice controls only the target mapping
applied to the empirical features. For a generic task, the historical MLP/tree
SCM generation remains in place. The new composition mode therefore changes
only informed tasks and preserves generic synthetic diversity through the
ratio parameter.

Feature-block coupling is fixed to zero and, in the new informed branch, is
bypassed before it could alter the empirical table. The empirical templates
already provide composition structure, and adding an artificial shared latent
component would undermine the purpose of retaining the physical features.

\subsection{Revised Optuna Configuration}

The revised pipeline uses the separate study

\[
\texttt{epit\_pipeline\_optuna\_empirical\_features\_scm\_target\_v6}.
\]

This prevents the new trials from being mixed with the v5 EPIT-rule study,
Trial~25, or any other historical study.

Only three parameters remain in the Optuna search:

\begin{itemize}
    \item \texttt{informed\_prior\_ratio};
    \item \texttt{mlp\_prob}, which controls the MLP/tree mixture; and
    \item \texttt{pitting\_composition\_perturb\_strength},
    with \(\tau\in[0,0.15]\).
\end{itemize}

The remaining relevant settings are fixed as follows:
...
& \texttt{empirical\_features\_scm\_target} \\
alloy families
& Fe and Ni--Cr--Mo, \(298:17\) \\
physical feature count
& \(21\) \\
target-head topology
& direct non-causal MLP/tree \\
target-head input scaling
& per-dataset column population z-score \\
constant-column target input
& zero \\
physical features modified by target head
& no \\
\texttt{informed\_feature\_block\_strength}
& \(0\) \\
\texttt{informed\_physical\_marginal\_prob}
& \(1\) \\
target policy
& SCM target from informed features \\
\texttt{pitting\_material\_dirichlet\_prob}
& \(0\) \\
...
Torch seed
& \(42\) \\
\texttt{prior\_n\_jobs}
& \(1\) \\
\hline
\end{tabular}
\end{center}

Magpie descriptors are fixed off during the base Optuna comparison. They remain
available as an explicit final-run override. When enabled, ten Magpie
descriptors are appended after the SCM target has been generated from the 21
physical columns, giving 31 model inputs. The descriptors are deterministic
functions of the composition and do not replace the empirical features.

The Dirichlet mechanism is disabled because it is unnecessary on top of real
templates. The physical feature profile is always active, and feature
permutation remains disabled. The study fingerprint records the feature
profile, composition profile, seeds, worker configuration, and artifact
checksums.

Final training automatically selects the finite completed trial with the
highest objective value at the time it starts. RUNNING or WAITING Optuna trials
do not block final training; their identifiers and states are recorded in the
final manifest. This is a snapshot decision: a later Optuna trial can become
the new study best, but it cannot silently change an already frozen final run.

For controlled experiments, final training can override all three searched
parameters as well as the fixed Magpie setting and composition-family weights.
Seeds, worker counts, training length, checkpoint intervals, and evaluation
settings remain configurable through the final launcher. The manifest records
the selected trial parameters separately from the effective training
parameters. An override configuration receives a distinct hashed run name and
output directory so it cannot be confused with the unmodified Optuna winner.

\subsection{Implementation Validation}

A generator audit using \(100{,}000\) synthetic rows produced

\[
94.555\%
\quad\text{Fe-alloy rows}
\]
...
\[
5.445\%
\quad\text{Ni--Cr--Mo rows},
\]

which closely matched the expected template proportions
...
The complete 24-element compositions closed to \(100\) weight percent with a
maximum numerical error below

\[
5\times10^{-14}.
\]

All generated composition values were non-negative.

The measured correlation between the sampled composition family and the family
associated with the sampled environment row was approximately

\[
-0.0008,
\]

confirming that composition and environment were sampled independently. At the
same time, every sampled temperature--chloride--pH--test-method tuple exactly
matched one record in the empirical profile.

Both the MLP and tree target branches were tested with the same generated
physical feature table. They returned finite, nonconstant targets while leaving
every physical feature value unchanged. For variable columns, the target-head
copy had approximately zero mean and unit population standard deviation.
Constant columns became zero only in that copy. The stored metadata confirmed
the policy

\[
\texttt{scm\_target\_from\_standardized\_physical\_features\_v1}
\]

and recorded that the physical features were not modified.

Reduced end-to-end training completed successfully for both the MLP and tree
informed target heads. Mixed generic/informed generation with two spawned data
loader workers also completed twice with identical seeded outputs. The full
generated v6 command parsed through the real training configuration, and a
Magpie override produced finite tensors with 31 model features while the target
head continued to consume the original 21 physical features.

The repository-wide regression suite passed 278 tests with two skipped when the
single pre-existing test for a historically deleted baseline launcher was
excluded. Historical generic generation remained unchanged, and the
\texttt{legacy}, \texttt{fe\_ni\_softmax}, and v5 empirical-feature modes
retained their separate behavior.

\subsection{Limitations}

The revised generator is not a thermodynamic, phase-stability, heat-treatment,
microstructure, or passivation model. Its composition diversity is bounded by
the patterns represented in the existing EPIT template bank.

The available family frequencies and compositions are also determined by the
current dataset. In particular, the Ni--Cr--Mo family contains only 17 distinct
templates and therefore remains substantially less diverse than the Fe-alloy
family.

The MLP/tree target is deliberately a synthetic nonlinear relationship rather
than a calibrated physical EPIT equation. Its purpose is to make every informed
target a meaningful function of the generated informed features while
retaining the diversity of SCM pretraining. The relationship should therefore
not be interpreted as a mechanistic corrosion law.

Nevertheless, the final v6 design removes the main inconsistency in the earlier
pipeline. The informed physical columns are generated first, retained
unchanged, and then used directly by an MLP feed-forward or tree SCM target
head. Generic tasks remain available through
\texttt{informed\_prior\_ratio}, so physical domain structure and generic
synthetic diversity coexist within the same pretraining workflow.



\section{Final v7 Correction of the SCM--EPIT Target Construction}
\label{sec:final-v7-scm-epit-correction}

This section supersedes the target-generation and preprocessing description in
the preceding v6 section. The empirical Fe and Ni--Cr--Mo physical-feature
generator introduced in v6 is retained. The correction concerns how those
features are presented to the SCM and transformer, how the synthetic target is
constructed, and how the corresponding parameters are searched.

\subsection{Problem in the v6 Implementation}

The intended design was to combine two complementary target components:

\begin{enumerate}
    \item a stochastic MLP- or tree-SCM target generated from the empirical
    physical features; and
    \item a calibrated corrosion correction generated by one of the existing
    analytical EPIT target rules.
\end{enumerate}

The v6 implementation included only the first component. After generating the
empirical physical feature table, it passed a standardized copy through the
selected SCM target head and used the resulting SCM output as the complete
informed target. The seven calibrated EPIT rules were not activated, their
scores and coefficients were omitted from the training command, and
\texttt{informed\_target\_mix\_weight} was fixed rather than searched.

Consequently, the informed feature generator was physically specialized, but
the target remained an arbitrary randomly sampled nonlinear function. The SCM
ensured that the target depended on the generated features, but it did not
ensure that the dependence contained the calibrated EPIT tendencies developed
in the preceding target-rule analysis. In particular, the target no longer
contained an explicit material-passivity, environmental-aggressiveness,
material--environment interaction, or test-method correction.

A second mismatch occurred during preprocessing. The SCM target head received
a population-standardized copy of the empirical physical features, but the
feature table subsequently passed through the general
\texttt{Reg2Cls} preprocessing path before reaching the transformer. Thus, the
SCM and transformer did not necessarily receive the same numerical
representation. This was inconsistent with the intended direct mapping

\[
X_{\mathrm{informed}}
\longrightarrow
f_{\theta}(X_{\mathrm{informed}})
\longrightarrow
y_{\mathrm{SCM}},
\]

because the target was calculated from one representation while the
transformer was trained on another.

Finally, the searched parameter \texttt{mlp\_prob} was assigned to both the
informed and generic MLP/tree mixtures. It should control only the choice of
target head for informed tasks. The generic part of the hybrid prior should
retain its established MLP/tree probabilities independently.

\subsection{Corrected Feature Representations}

Let

\[
X_{\mathrm{raw}}\in\mathbb{R}^{n\times 21}
\]

denote one empirical physical EPIT table. Its columns retain the fixed schema
...
The existing physical generator is unchanged. Alloy compositions are still
sampled from the empirical Fe and Ni--Cr--Mo template bank, perturbed
multiplicatively, closed in the complete source composition space, and then
projected to the 17 observed elements. Temperature, chloride concentration,
pH, and test method are still sampled as complete empirical environmental
tuples.

A separate standardized representation is now constructed using the
population mean and standard deviation of each column within the generated
task:
...
the raw physical values because they contain physically meaningful operations
such as composition thresholds, chromium--molybdenum terms, logarithmic
chloride transformations, temperature thresholds, pH corrections, and
test-method category lookups.

For informed
\texttt{empirical\_features\_scm\_target} tasks, the general
\texttt{Reg2Cls} preprocessing operation is therefore bypassed. The explicitly
constructed \(X_{\mathrm{std}}\) is passed directly to the transformer.
Constant columns become zero but are not deleted, so the fixed column
positions remain valid.

This change is restricted to the informed SCM--EPIT branch. Generic tasks
continue to use the existing \texttt{Reg2Cls} path, including its established
feature and target preprocessing.

\subsection{Corrected SCM--EPIT Target}

The standardized empirical features are first passed through the selected
direct non-causal target head:

\[
y_{\mathrm{SCM}}
=
f_{\theta}^{\mathrm{MLP/tree}}
\left(X_{\mathrm{std}}\right).
\]

The MLP and tree implementations are unchanged. The supplied 21 columns are
treated as external causes, and the target head produces one synthetic
response per row without generating a replacement feature table.

Independently, one calibrated analytical EPIT rule \(g_k\) is sampled and
evaluated on the raw physical table:

\[
y_{\mathrm{EPIT}}
=
g_k\left(X_{\mathrm{raw}}\right).
\]

The available rule families are

\begin{itemize}
    \item \texttt{pren\_linear};
    \item \texttt{cr\_mow\_synergy};
    \item \texttt{threshold\_saturation};
    \item \texttt{improved\_environment};
    \item \texttt{coupled\_breakdown};
    \item \texttt{fe\_ni\_cr\_threshold}; and
    \item \texttt{method\_aware}.
\end{itemize}

These are corrosion-target rule families. They are distinct from the Fe and
Ni--Cr--Mo alloy families used by the physical-feature generator.

Let \(s_k>0\) be the saved calibration score of rule \(k\). The rule is sampled
with probability

\[
p_k
=
\frac{s_k}{\sum_{\ell=1}^{7}s_\ell}.
\]

The selected rule uses its saved calibrated coefficient vector. Across the
seven rules, the training command supplies all seven scores and all 29
coefficient values. The v7 command serializes these values with
round-trip-exact floating-point precision, so the values consumed during
training are identical to those recorded in the calibration artifacts and
pipeline fingerprint. The informed SCM--EPIT mode fails immediately if the
calibrated rule scores are absent rather than silently reverting to the older
uncalibrated target rule.

The final informed target is controlled by
...
Both components are standardized before mixing, and the combined target is
standardized once more:

\begin{equation}
\label{eq:final-v7-scm-epit-target}
y_{\mathrm{informed}}
=
\operatorname{zscore}
\left[
(1-\lambda)
\operatorname{zscore}(y_{\mathrm{SCM}})
+
\lambda
\operatorname{zscore}(y_{\mathrm{EPIT}})
\right].
\end{equation}

This construction gives the parameter a direct interpretation:
...
\lambda=1
\quad\Longrightarrow\quad
\text{pure calibrated EPIT-rule target},
\]

and intermediate values combine stochastic SCM diversity with the calibrated
corrosion correction. Final target standardization prevents the absolute scale
of either component from determining the regression loss and removes any
literal millivolt interpretation from the synthetic response.

\subsection{Generic and Informed MLP/Tree Mixtures}

The hybrid prior still samples informed tasks with probability
...
For informed tasks, \texttt{mlp\_prob} controls whether
\(f_{\theta}^{\mathrm{MLP}}\) or \(f_{\theta}^{\mathrm{tree}}\) generates
\(y_{\mathrm{SCM}}\). It has no effect on whether the task is informed.

Generic tasks retain the original fixed mixture

\[
P(\mathrm{MLP}\mid\mathrm{generic})=0.7,
\qquad
P(\mathrm{tree}\mid\mathrm{generic})=0.3.
\]

Thus, varying the informed MLP/tree mixture no longer changes the generic
control distribution.

\subsection{Optional Magpie Representation}

Magpie descriptors remain disabled in the base Optuna search but are available
as a recorded final-training override. When enabled, the ten deterministic
descriptors are calculated from the raw empirical composition and appended to
the physical table:
...
The complete 31-column table is then standardized column by column before it
is passed to the transformer. Because this operation is column-wise, its first
21 columns are numerically identical to the standardized 21-column table used
by the SCM:
...
The SCM target head continues to use only the 21 physical features, and the
analytical EPIT rule continues to use their raw physical representation.
Magpie therefore adds deterministic composition information for the
transformer without changing either target mechanism.

\subsection{Revised v7 Optuna Study}

The corrected implementation uses a new study:

\[
\texttt{epit\_pipeline\_optuna\_empirical\_features\_scm\_target\_v7}.
\]

A separate study is required because the v6 trials were trained using a
different target policy and preprocessing path. Reusing their objective values
would mix incompatible experiments.

The v7 search contains four active dimensions:

\begin{itemize}
    \item \texttt{informed\_prior\_ratio};
    \item \texttt{mlp\_prob}, applied only to informed target heads;
    \item \texttt{pitting\_composition\_perturb\_strength},
    with values in \([0,0.15]\); and
    \item \texttt{informed\_target\_mix\_weight},
    sampled continuously in \([0,1]\).
\end{itemize}

The calibrated rule scores, rule probabilities, rule coefficients, empirical
feature profile, generic MLP/tree mixture, fixed schema, preprocessing policy,
seeds, and worker configuration are recorded in the study fingerprint.
Stage~4 reconstructs \(\lambda\) from the selected Optuna trial and passes the
same target-rule artifacts to final training.

\subsection{Scope of the Correction}

The correction changes only the informed
\texttt{empirical\_features\_scm\_target} branch. It does not alter

\begin{itemize}
    \item the empirical Fe/Ni--Cr--Mo feature generator;
    \item the calibrated target-rule formulas or their fitted coefficients;
    \item the generic SCM generator or its \texttt{Reg2Cls} preprocessing;
    \item the historical \texttt{legacy},
    \texttt{fe\_ni\_softmax}, or
    \texttt{empirical\_features} modes;
    \item the frozen development/final-test split;
    \item Optuna fold evaluation;
    \item checkpoint generation or checkpoint selection; or
    \item final-test evaluation.
\end{itemize}

Direct comparisons with the preceding source revision confirmed that the
parameters, generated commands, fingerprints, and seeded outputs of the
generic and historical informed modes remain unchanged.

\subsection{Implementation Validation}

The corrected target equation was verified independently for all seven
calibrated rule families at

\[
\lambda\in\{0,\;0.37,\;1\}.
\]

End-to-end generation was additionally tested for both MLP and tree target
heads, with 21 physical features and with the 31-feature Magpie override, and
at the two target endpoints and an intermediate mixture. In every tested case,

\begin{itemize}
    \item the analytical rule received the raw physical table;
    \item the SCM and transformer received identical standardized physical
    columns;
    \item the generated features and targets were finite;
    \item the final target had approximately zero mean and unit population
    standard deviation;
    \item the saved rule scores and coefficients were reconstructed exactly;
    \item the generic \texttt{Reg2Cls} path was not called for the informed
    SCM--EPIT task; and
    \item repeated generation with identical NumPy, Torch, and Python seeds
    produced identical features, targets, and selected rule families.
\end{itemize}

The complete Optuna-to-final-training path was also checked from parameter
sampling through stored trial reconstruction and parsing by the real training
configuration. The repository-wide regression suite passed 283 tests with two
skipped after excluding one pre-existing test whose historical baseline Slurm
launcher is no longer present.

The final v7 construction therefore implements the intended division of
responsibilities: the empirical generator supplies physically structured
features, the SCM supplies stochastic nonlinear task diversity, the calibrated
EPIT rule supplies corrosion-specific target direction, and
\(\lambda\) determines their relative importance.


\section{Final v7 EPIT Prior and Held-Out Evaluation}
\label{sec:final-v7-results}

The v6 empirical-feature implementation generated realistic physical input
tables but obtained the synthetic target only from the MLP or tree SCM.
Consequently, the pitting-specific target-family mechanism and its calibrated
EPIT rules were effectively bypassed. Version~7 corrects this inconsistency
while retaining the same 21-feature empirical input schema.

For each informed synthetic task, v7 combines a standardized SCM target
\(y_{\mathrm{SCM}}\) with a standardized calibrated EPIT-rule drive
\(g_{\mathrm{EPIT}}(X)\):
\[
y_{\mathrm{synthetic}}
=
\operatorname{standardize}
\left[
(1-\lambda)\operatorname{standardize}(y_{\mathrm{SCM}})
+
\lambda\operatorname{standardize}\!\left(g_{\mathrm{EPIT}}(X)\right)
\right].
\]
The mixture weight \(\lambda\) is optimized rather than fixed. The EPIT rule is
sampled using the development-calibrated rule probabilities described in
Section~\ref{sec:epit-rule-probabilities}. No final-test targets enter this
process.

\subsection{Optuna Selection}

The v7 study was named
\texttt{epit\_pipeline\_optuna\_empirical\_features\_scm\_target\_v7}.
At the time of final-model freezing, the study contained 78 trials, of which
six were still marked as running. Final training explicitly selected the best
completed trial available at that time, Trial~56. Its mean five-fold
development Spearman correlation was \(0.6903\), with mean MAE
\(211.0\)~mV, RMSE \(298.0\)~mV, and \(R^2=0.4373\).

Trial~56 used
\[
\rho=0.75,\qquad
P(\mathrm{MLP})=0.50,\qquad
\lambda=0.2048,\qquad
\tau=0.1028,
\]
where \(\rho\) is the informed-task probability, \(\lambda\) is the EPIT target
mixture weight, and \(\tau\) is the empirical-composition perturbation
strength. Magpie descriptors were disabled.

\subsection{Final Training and Checkpoint Selection}

Trial~56 was retrained from scratch for \(10{,}000\) Stage~1 steps. Only the
608 development rows were used during model development. Permanent
checkpoints were saved every 500 steps and evaluated using the five frozen
development folds.

Checkpoint selection used mean development-fold Spearman correlation. The
selected and cryptographically locked checkpoint was step~\(2{,}500\), which
obtained mean development-fold Spearman \(0.6966\), MAE \(191.3\)~mV,
RMSE \(280.9\)~mV, and \(R^2=0.4717\). The final-model manifest records the
selected trial, training configuration, split hashes, checkpoint evaluations,
and selected-checkpoint hash.

\subsection{Final Held-Out Results}

The locked step-\(2{,}500\) checkpoint was evaluated using all 608 development
rows as context and the 152 composition-isolated final-test rows as queries.
Table~\ref{tab:v7-final-results} reports the results. The generic local model
is shown at the matching training step; pretrained TabICL V2 and CatBoost are
checkpoint-independent references.

\begin{table}[htbp]
\centering
\caption{Performance on the 152-row held-out EPIT test set.}
\label{tab:v7-final-results}
\begin{tabular}{lrrrr}
\hline
Model & Spearman & MAE (mV) & RMSE (mV) & \(R^2\) \\
\hline
Trial~56, step 2500 & \textbf{0.7681} & \textbf{186.8}
                    & \textbf{268.7} & \textbf{0.5294} \\
Pretrained TabICL V2 & 0.6011 & 206.8 & 343.8 & 0.2294 \\
CatBoost              & 0.6090 & 231.7 & 323.1 & 0.3194 \\
Generic baselinetrained baseline, step 2500
                      & 0.2829 & 326.4 & 548.8 & -0.9630 \\
\hline
\end{tabular}
\end{table}

Relative to pretrained TabICL V2, Trial~56 reduced MAE by \(9.7\%\), reduced
RMSE by \(21.9\%\), and increased Spearman correlation by \(0.1670\).
Its final-test performance is also broadly consistent with its development-fold
performance, although only one final training seed was evaluated.
...
checkpoint as the reported confirmatory model. The held-out results establish
generalization to composition-isolated rows from the same source dataset, not
to independent publications, laboratories, or alloy databases.

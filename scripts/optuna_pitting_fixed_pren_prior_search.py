#!/usr/bin/env python
"""Tune fixed-schema PREN-informed prior parameters for the alloy pitting task.

Each trial trains one TabICL regression checkpoint with the same fixed 21-feature
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--work-dir",
        type=Path,
        default=REPO_ROOT / "corrosion_datasets" / "analysis" / "pitting_fixed_pren_prior_search",
    )
    parser.add_argument(
        "--eval-output-root",
        type=Path,
...
def training_command(args: argparse.Namespace, params: TrialParams, checkpoint_dir: Path) -> list[str]:
    mlp_prob = float(params.mlp_prob)
    ...
    command = [
        ...
        *(str(value) for value in FIXED_PREN_BLOCK_ALLOCATION),
        ...
        *(format_float(value) for value in DEFAULT_INHIBITOR_ALLOCATION),
        ...
        str(FIXED_PREN_PROCESS_CATEGORY_COUNT),
...
def run_trial(args: argparse.Namespace, trial_number: int, params: TrialParams) -> dict[str, Any]:
    ...
    eval_dir = study_output_root(args.eval_output_root, args.study_name) / f"pitting_fixed_pren_repeated_splits_{trial_name}"
    ...
    metadata = {
        ...
        "fixed_normal_block_allocation": FIXED_PREN_BLOCK_ALLOCATION,
        "fixed_normal_block_allocation_min_counts": FIXED_PREN_BLOCK_ALLOCATION,
        "fixed_feature_count": FIXED_EPIT_FEATURE_COUNT,
